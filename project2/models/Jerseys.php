...
/**
 * Table class for jerseys
 */
#[AsAlias("Jerseys", true)]
#[AsAlias("jerseys", true)]
#[AsEntityListener(event: Events::postPersist, method: 'clearCache', entity: Entity\Jersey::class)]
#[AsEntityListener(event: Events::postUpdate, method: 'clearCache', entity: Entity\Jersey::class)]
#[AsEntityListener(event: Events::postRemove, method: 'clearCache', entity: Entity\Jersey::class)]
#[AsEntityListener(event: Events::postPersist, method: 'updateLastInsertId', entity: Entity\Jersey::class)]
#[AsEntityListener(event: Events::preUpdate, method: 'preUpdate', entity: Entity\Jersey::class)]
class Jerseys extends DbTable implements LookupTableInterface
{
    ...
    public function __construct(
        ...
        $this->EntityClass = Entity\Jersey::class;
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // jersey_name
        $this->jersey_name = $this->Fields['jersey_name'];
    ...
    public function getFieldDefinitions(): array
    {
        return [
            ...
            'jersey_name' => [
                'FieldVar' => 'x_jersey_name', // Field variable name
                'Param' => 'jersey_name', // Field parameter name (Table class property name)
                'PropertyName' => 'jerseyName', // Field entity property name
                'Expression' => '`jersey_name`', // Field expression (used in SQL)
                'BasicSearchExpression' => '`jersey_name`', // Field expression (used in basic search SQL)
                'Type' => 200, // Field type
                'DataType' => DataType::STRING, // Field data type (DataType::XXX)
                'ParameterType' => ParameterType::STRING, // Field Doctrine parameter type
                'Size' => 100, // Field size
                'DateTimeFormat' => -1, // Date time format
                'VirtualExpression' => '`jersey_name`', // Virtual field expression (used in ListSQL)
                'IsVirtual' => false, // Virtual field
                'ForceSelection' => false, // Autosuggest force selection
                'VirtualSearch' => false, // Search as virtual field
                'ViewTag' => 'FORMATTED TEXT', // View Tag
     ...
     */
    public function clearCache(Entity\Jersey $row, LifecycleEventArgs $args): void
    {
        // Clear lookup cache
    ...
    protected function loadDbValues(?BaseEntity $row)
    {
        ...
        $this->jersey_name->DbValue = $row->getJerseyName();
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("JerseysList");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "JerseysView" => $this->language->phrase("View"),
            "JerseysEdit" => $this->language->phrase("Edit"),
            "JerseysAdd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getViewUrl(string $param = ""): string
    {
        ...
        $url = $this->keyUrl("JerseysView", $params);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("JerseysView", [$param]);
        return $this->addMasterUrl($url);
    }
    ...
    public function getEditUrl(string $param = ""): string
    {
        ...
        $url = $this->keyUrl("JerseysEdit", $params);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("JerseysList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("JerseysEdit", [$param]);
        return $this->addMasterUrl($url);
    }
    ...
    public function getCopyUrl(string $param = ""): string
    {
        ...
        $url = $this->keyUrl("JerseysAdd", $params);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("JerseysList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $param = ""): string
    {
        if ($this->UseAjaxActions && IsInfiniteScroll() && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("JerseysDelete", $param);
        }
    }
    ...
    public function renderListContent(array|BaseEntity $data, int $totalRecords)
    {
        global $httpContext;
        $container = Container();
        $page = $container->get("JerseysList");
        if (is_array($data)) {
        ...
        if (!$doc->ExportCustom && $writeHeader) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->jersey_name);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->jersey_name);
    ...
    public function updateLastInsertId(Entity\Jersey $row, LifecycleEventArgs $args)
    {
        $this->id->setDbValue($row['id']);
    }

    // Save entity change set
    public function preUpdate(Entity\Jersey $row, PreUpdateEventArgs $args): void
    {
    ...
    public function preRemove(Entity\Jersey $row, LifecycleEventArgs $args): void
    {
