...
#[AsAlias("Users", true)]
#[AsAlias("users", true)]
#[AsEntityListener(event: Events::postPersist, method: 'clearCache', entity: Entity\User::class)]
#[AsEntityListener(event: Events::postUpdate, method: 'clearCache', entity: Entity\User::class)]
#[AsEntityListener(event: Events::postRemove, method: 'clearCache', entity: Entity\User::class)]
#[AsEntityListener(event: Events::postPersist, method: 'updateLastInsertId', entity: Entity\User::class)]
#[AsEntityListener(event: Events::preUpdate, method: 'preUpdate', entity: Entity\User::class)]
class Users extends DbTable implements LookupTableInterface
{
    ...
    // Fields
    public DbField $id;
    public DbField $fullname;
    public DbField $email;
    public DbField $contact;
    ...
    public function __construct(
        ...
        // contact
        $this->contact = $this->Fields['contact'];
    ...
    public function getFieldDefinitions(): array
    {
        return [
            ...
            'fullname' => [
                'FieldVar' => 'x_fullname', // Field variable name
                'Param' => 'fullname', // Field parameter name (Table class property name)
                'PropertyName' => 'fullname', // Field entity property name
                'Expression' => '`fullname`', // Field expression (used in SQL)
                'BasicSearchExpression' => '`fullname`', // Field expression (used in basic search SQL)
                'Type' => 200, // Field type
                'DataType' => DataType::STRING, // Field data type (DataType::XXX)
                'ParameterType' => ParameterType::STRING, // Field Doctrine parameter type
                'Size' => 100, // Field size
                'DateTimeFormat' => -1, // Date time format
                'VirtualExpression' => '`fullname`', // Virtual field expression (used in ListSQL)
                'IsVirtual' => false, // Virtual field
                'ForceSelection' => false, // Autosuggest force selection
                'VirtualSearch' => false, // Search as virtual field
                'ViewTag' => 'FORMATTED TEXT', // View Tag
            ...
            'contact' => [
                'FieldVar' => 'x_contact', // Field variable name
                'Param' => 'contact', // Field parameter name (Table class property name)
                'PropertyName' => 'contact', // Field entity property name
                'Expression' => '`contact`', // Field expression (used in SQL)
                'BasicSearchExpression' => '`contact`', // Field expression (used in basic search SQL)
                'Type' => 200, // Field type
                'DataType' => DataType::STRING, // Field data type (DataType::XXX)
                'ParameterType' => ParameterType::STRING, // Field Doctrine parameter type
                'Size' => 20, // Field size
                'DateTimeFormat' => -1, // Date time format
                'VirtualExpression' => '`contact`', // Virtual field expression (used in ListSQL)
                'IsVirtual' => false, // Virtual field
                'ForceSelection' => false, // Autosuggest force selection
                'VirtualSearch' => false, // Search as virtual field
                'ViewTag' => 'FORMATTED TEXT', // View Tag
     ...
     */
    public function clearCache(Entity\User $row, LifecycleEventArgs $args): void
    {
        // Clear lookup cache
    ...
    protected function loadDbValues(?BaseEntity $row)
    {
        ...
        $this->fullname->DbValue = $row->getFullname();
        $this->email->DbValue = $row->getEmail();
        $this->contact->DbValue = $row->getContact();
    ...
    public function getViewUrl(string $param = ""): string
    {
        ...
        $url = $this->keyUrl("UsersView", $params);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("UsersView", [$param]);
        return $this->addMasterUrl($url);
    }
    ...
    public function getEditUrl(string $param = ""): string
    {
        ...
        $url = $this->keyUrl("UsersEdit", $params);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("UsersEdit", [$param]);
        return $this->addMasterUrl($url);
    }
    ...
    public function getCopyUrl(string $param = ""): string
    {
        ...
        $url = $this->keyUrl("UsersAdd", $params);
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $param = ""): string
    {
        if ($this->UseAjaxActions && IsInfiniteScroll() && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("UsersDelete", $param);
        }
    }
    ...
    public function updateLastInsertId(Entity\User $row, LifecycleEventArgs $args)
    {
        $this->id->setDbValue($row['id']);
    }

    // Save entity change set
    public function preUpdate(Entity\User $row, PreUpdateEventArgs $args): void
    {
    ...
    public function preRemove(Entity\User $row, LifecycleEventArgs $args): void
    {
