 ...
 * Page class
 */
#[AsAlias("JerseysAdd", true)]
class JerseysAdd extends Jerseys implements PageInterface
{
    use MessagesTrait;
    use FormTrait;
    ...
    // CSS class/style
    public string $CurrentPageName = "JerseysAdd"; // Route action
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$oldRow) { // Record not loaded
                    ...
                    $this->terminate("JerseysList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                if ($newRow = $this->addRow($oldRow)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "JerseysList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "JerseysView") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "JerseysList") {
                            ...
                            $returnUrl = "JerseysList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate();
                    } else {
                        if ($this->IsModal && GetPageName($returnUrl) != "JerseysList") {
                            $this->IsModal = false;
                            FlashBag()->add("X-Refresh-Url", GetUrl("JerseysList")); // Refresh page after add before going to return page
    ...
    protected function loadFormValues(): void
    {
        ...
        // jersey_name
        if (!$this->jersey_name->IsDetailKey) {
            $val = $this->hasInputValue($this->jersey_name) ? $this->getInputValue($this->jersey_name) : null;
            $this->jersey_name->setFormValue($val);
        }
    ...
    {
        $this->jersey_name->CurrentValue = $this->jersey_name->FormValue;
    ...
    public function renderRow(RowType $rowType = RowType::VIEW, bool $resetAttributes = true): void
    {
        ...
        // jersey_name
        $this->jersey_name->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // jersey_name
            $this->jersey_name->setupEditAttributes();
            $this->jersey_name->EditValue = !$this->jersey_name->Raw ? HtmlDecode($this->jersey_name->CurrentValue) : $this->jersey_name->CurrentValue;
            $this->jersey_name->PlaceHolder = RemoveHtml($this->jersey_name->caption());
    ...
    protected function validateForm(): bool
    {
        ...
        $validateForm = true;
        if ($this->jersey_name->Visible) {
            if ($this->jersey_name->Required) {
                if (!$this->jersey_name->IsDetailKey && IsEmpty($this->jersey_name->FormValue)) {
                    $this->jersey_name->addErrorMessage(str_replace("%s", $this->jersey_name->caption(), $this->jersey_name->RequiredErrorMessage));
                }
            }
        }
    ...
    protected function getAddRow(): object
    {
        ...
        // jersey_name
        $newRow->setJerseyName($this->jersey_name->setDbValueDef($this->jersey_name->CurrentValue));
