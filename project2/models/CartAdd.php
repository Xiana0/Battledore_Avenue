 ...
 * Page class
 */
#[AsAlias("CartAdd", true)]
class CartAdd extends Cart implements PageInterface
{
    use MessagesTrait;
    use FormTrait;
    ...
    // CSS class/style
    public string $CurrentPageName = "CartAdd"; // Route action
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$oldRow) { // Record not loaded
                    ...
                    $this->terminate("CartList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                if ($newRow = $this->addRow($oldRow)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "CartList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "CartView") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "CartList") {
                            ...
                            $returnUrl = "CartList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate();
                    } else {
                        if ($this->IsModal && GetPageName($returnUrl) != "CartList") {
                            $this->IsModal = false;
                            FlashBag()->add("X-Refresh-Url", GetUrl("CartList")); // Refresh page after add before going to return page
    ...
    protected function loadFormValues(): void
    {
        ...
        if (!$this->product_type->IsDetailKey) {
            $val = $this->hasInputValue($this->product_type) ? $this->getInputValue($this->product_type) : null;
    ...
    public function renderRow(RowType $rowType = RowType::VIEW, bool $resetAttributes = true): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->product_type->EditValue = !$this->product_type->Raw ? HtmlDecode($this->product_type->CurrentValue) : $this->product_type->CurrentValue;
    ...
    protected function validateForm(): bool
    {
        ...
        if ($this->product_type->Visible) {
            if ($this->product_type->Required) {
                if (!$this->product_type->IsDetailKey && IsEmpty($this->product_type->FormValue)) {
    ...
    protected function getAddRow(): object
    {
        ...
        // product_type
        $newRow->setProductType($this->product_type->setDbValueDef($this->product_type->CurrentValue));

        // quantity
        if (!IsEmpty(strval($this->quantity->CurrentValue))) {
