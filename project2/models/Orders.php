...
#[AsAlias("Orders", true)]
#[AsAlias("orders", true)]
#[AsEntityListener(event: Events::postPersist, method: 'clearCache', entity: Entity\Order::class)]
#[AsEntityListener(event: Events::postUpdate, method: 'clearCache', entity: Entity\Order::class)]
#[AsEntityListener(event: Events::postRemove, method: 'clearCache', entity: Entity\Order::class)]
#[AsEntityListener(event: Events::postPersist, method: 'updateLastInsertId', entity: Entity\Order::class)]
#[AsEntityListener(event: Events::preUpdate, method: 'preUpdate', entity: Entity\Order::class)]
class Orders extends DbTable implements LookupTableInterface
{
    ...
    public DbField $total_amount;
    public DbField $payment_method;
    ...
    public function __construct(
        ...
        $this->EntityClass = Entity\Order::class;
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // total_amount
        $this->total_amount = $this->Fields['total_amount'];
        $this->total_amount->DefaultErrorMessage = $this->language->phrase("IncorrectFloat");

        // payment_method
        $this->payment_method = $this->Fields['payment_method'];
    ...
    public function getFieldDefinitions(): array
    {
        return [
            ...
            'total_amount' => [
                'FieldVar' => 'x_total_amount', // Field variable name
                'Param' => 'total_amount', // Field parameter name (Table class property name)
                'PropertyName' => 'totalAmount', // Field entity property name
                'Expression' => '`total_amount`', // Field expression (used in SQL)
                'BasicSearchExpression' => '`total_amount`', // Field expression (used in basic search SQL)
                'Type' => 131, // Field type
                'DataType' => DataType::NUMBER, // Field data type (DataType::XXX)
                'ParameterType' => ParameterType::STRING, // Field Doctrine parameter type
                'Size' => 12, // Field size
                'DateTimeFormat' => -1, // Date time format
                'VirtualExpression' => '`total_amount`', // Virtual field expression (used in ListSQL)
                'IsVirtual' => false, // Virtual field
                'ForceSelection' => false, // Autosuggest force selection
                'VirtualSearch' => false, // Search as virtual field
                'ViewTag' => 'FORMATTED TEXT', // View Tag
            ...
            'payment_method' => [
                'FieldVar' => 'x_payment_method', // Field variable name
                'Param' => 'payment_method', // Field parameter name (Table class property name)
                'PropertyName' => 'paymentMethod', // Field entity property name
                'Expression' => '`payment_method`', // Field expression (used in SQL)
                'BasicSearchExpression' => '`payment_method`', // Field expression (used in basic search SQL)
                'Type' => 200, // Field type
                'DataType' => DataType::STRING, // Field data type (DataType::XXX)
                'ParameterType' => ParameterType::STRING, // Field Doctrine parameter type
                'Size' => 50, // Field size
                'DateTimeFormat' => -1, // Date time format
                'VirtualExpression' => '`payment_method`', // Virtual field expression (used in ListSQL)
                'IsVirtual' => false, // Virtual field
                'ForceSelection' => false, // Autosuggest force selection
                'VirtualSearch' => false, // Search as virtual field
                'ViewTag' => 'FORMATTED TEXT', // View Tag
     ...
     */
    public function clearCache(Entity\Order $row, LifecycleEventArgs $args): void
    {
        // Clear lookup cache
    ...
    protected function loadDbValues(?BaseEntity $row)
    {
        ...
        $this->total_amount->DbValue = $row->getTotalAmount();
        $this->payment_method->DbValue = $row->getPaymentMethod();
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("OrdersList");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "OrdersView" => $this->language->phrase("View"),
            "OrdersEdit" => $this->language->phrase("Edit"),
            "OrdersAdd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getViewUrl(string $param = ""): string
    {
        ...
        $url = $this->keyUrl("OrdersView", $params);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("OrdersView", [$param]);
        return $this->addMasterUrl($url);
    }
    ...
    public function getEditUrl(string $param = ""): string
    {
        ...
        $url = $this->keyUrl("OrdersEdit", $params);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("OrdersEdit", [$param]);
        return $this->addMasterUrl($url);
    }
    ...
    public function getCopyUrl(string $param = ""): string
    {
        ...
        $url = $this->keyUrl("OrdersAdd", $params);
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $param = ""): string
    {
        if ($this->UseAjaxActions && IsInfiniteScroll() && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("OrdersDelete", $param);
        }
    }
    ...
    public function updateLastInsertId(Entity\Order $row, LifecycleEventArgs $args)
    {
        $this->id->setDbValue($row['id']);
    }

    // Save entity change set
    public function preUpdate(Entity\Order $row, PreUpdateEventArgs $args): void
    {
    ...
    public function preRemove(Entity\Order $row, LifecycleEventArgs $args): void
    {
