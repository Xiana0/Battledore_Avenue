...
/**
 * Table class for accessories
 */
#[AsAlias("Accessories", true)]
#[AsAlias("accessories", true)]
#[AsEntityListener(event: Events::postPersist, method: 'clearCache', entity: Entity\Accessory::class)]
#[AsEntityListener(event: Events::postUpdate, method: 'clearCache', entity: Entity\Accessory::class)]
#[AsEntityListener(event: Events::postRemove, method: 'clearCache', entity: Entity\Accessory::class)]
#[AsEntityListener(event: Events::postPersist, method: 'updateLastInsertId', entity: Entity\Accessory::class)]
#[AsEntityListener(event: Events::preUpdate, method: 'preUpdate', entity: Entity\Accessory::class)]
class Accessories extends DbTable implements LookupTableInterface
{
    ...
    public function __construct(
        ...
        $this->EntityClass = Entity\Accessory::class;
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // accessory_name
        $this->accessory_name = $this->Fields['accessory_name'];

        // price
        $this->price = $this->Fields['price'];
    ...
    public function getFieldDefinitions(): array
    {
        return [
            ...
            'accessory_name' => [
                'FieldVar' => 'x_accessory_name', // Field variable name
                'Param' => 'accessory_name', // Field parameter name (Table class property name)
                'PropertyName' => 'accessoryName', // Field entity property name
                'Expression' => '`accessory_name`', // Field expression (used in SQL)
                'BasicSearchExpression' => '`accessory_name`', // Field expression (used in basic search SQL)
                'Type' => 200, // Field type
                'DataType' => DataType::STRING, // Field data type (DataType::XXX)
                'ParameterType' => ParameterType::STRING, // Field Doctrine parameter type
                'Size' => 100, // Field size
                'DateTimeFormat' => -1, // Date time format
                'VirtualExpression' => '`accessory_name`', // Virtual field expression (used in ListSQL)
                'IsVirtual' => false, // Virtual field
                'ForceSelection' => false, // Autosuggest force selection
                'VirtualSearch' => false, // Search as virtual field
                'ViewTag' => 'FORMATTED TEXT', // View Tag
            ...
            'price' => [
                'FieldVar' => 'x_price', // Field variable name
                'Param' => 'price', // Field parameter name (Table class property name)
                'PropertyName' => 'price', // Field entity property name
                'Expression' => '`price`', // Field expression (used in SQL)
                'BasicSearchExpression' => '`price`', // Field expression (used in basic search SQL)
                'Type' => 131, // Field type
                'DataType' => DataType::NUMBER, // Field data type (DataType::XXX)
                'ParameterType' => ParameterType::STRING, // Field Doctrine parameter type
                'Size' => 12, // Field size
                'DateTimeFormat' => -1, // Date time format
                'VirtualExpression' => '`price`', // Virtual field expression (used in ListSQL)
                'IsVirtual' => false, // Virtual field
                'ForceSelection' => false, // Autosuggest force selection
                'VirtualSearch' => false, // Search as virtual field
                'ViewTag' => 'FORMATTED TEXT', // View Tag
    ...
    {
        return $this->sqlFrom ?? "accessories";
    }
     ...
     */
    public function clearCache(Entity\Accessory $row, LifecycleEventArgs $args): void
    {
        // Clear lookup cache
    ...
    protected function loadDbValues(?BaseEntity $row)
    {
        ...
        $this->accessory_name->DbValue = $row->getAccessoryName();
        $this->price->DbValue = $row->getPrice();
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("AccessoriesList");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "AccessoriesView" => $this->language->phrase("View"),
            "AccessoriesEdit" => $this->language->phrase("Edit"),
            "AccessoriesAdd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getViewUrl(string $param = ""): string
    {
        ...
        $url = $this->keyUrl("AccessoriesView", $params);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AccessoriesView", [$param]);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = BuildUrl("AccessoriesAdd", $param);
        return $this->addMasterUrl($url);
    }
    ...
    public function getEditUrl(string $param = ""): string
    {
        ...
        $url = $this->keyUrl("AccessoriesEdit", $params);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AccessoriesList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AccessoriesEdit", [$param]);
        return $this->addMasterUrl($url);
    }
    ...
    public function getCopyUrl(string $param = ""): string
    {
        ...
        $url = $this->keyUrl("AccessoriesAdd", $params);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AccessoriesList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $param = ""): string
    {
        if ($this->UseAjaxActions && IsInfiniteScroll() && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("AccessoriesDelete", $param);
        }
    }
    ...
    public function renderListContent(array|BaseEntity $data, int $totalRecords)
    {
        global $httpContext;
        $container = Container();
        $page = $container->get("AccessoriesList");
        if (is_array($data)) {
        ...
        if (!$doc->ExportCustom && $writeHeader) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->accessory_name);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->accessory_name);
    ...
    public function updateLastInsertId(Entity\Accessory $row, LifecycleEventArgs $args)
    {
        $this->id->setDbValue($row['id']);
    }

    // Save entity change set
    public function preUpdate(Entity\Accessory $row, PreUpdateEventArgs $args): void
    {
    ...
    public function preRemove(Entity\Accessory $row, LifecycleEventArgs $args): void
    {
