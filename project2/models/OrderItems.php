...
/**
 * Table class for order_items
 */
#[AsAlias("OrderItems", true)]
#[AsAlias("order_items", true)]
#[AsEntityListener(event: Events::postPersist, method: 'clearCache', entity: Entity\OrderItem::class)]
#[AsEntityListener(event: Events::postUpdate, method: 'clearCache', entity: Entity\OrderItem::class)]
#[AsEntityListener(event: Events::postRemove, method: 'clearCache', entity: Entity\OrderItem::class)]
#[AsEntityListener(event: Events::postPersist, method: 'updateLastInsertId', entity: Entity\OrderItem::class)]
#[AsEntityListener(event: Events::preUpdate, method: 'preUpdate', entity: Entity\OrderItem::class)]
class OrderItems extends DbTable implements LookupTableInterface
{
    ...
    public function __construct(
        ...
        $this->EntityClass = Entity\OrderItem::class;
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // order_id
        $this->order_id = $this->Fields['order_id'];
        $this->order_id->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
    ...
    public function getFieldDefinitions(): array
    {
        return [
            ...
            'order_id' => [
                'FieldVar' => 'x_order_id', // Field variable name
                'Param' => 'order_id', // Field parameter name (Table class property name)
                'PropertyName' => 'orderId', // Field entity property name
                'Expression' => '`order_id`', // Field expression (used in SQL)
                'BasicSearchExpression' => '`order_id`', // Field expression (used in basic search SQL)
                'Type' => 3, // Field type
                'DataType' => DataType::NUMBER, // Field data type (DataType::XXX)
                'ParameterType' => ParameterType::INTEGER, // Field Doctrine parameter type
                'Size' => 11, // Field size
                'DateTimeFormat' => -1, // Date time format
                'VirtualExpression' => '`order_id`', // Virtual field expression (used in ListSQL)
                'IsVirtual' => false, // Virtual field
                'ForceSelection' => false, // Autosuggest force selection
                'VirtualSearch' => false, // Search as virtual field
                'ViewTag' => 'FORMATTED TEXT', // View Tag
    ...
    {
        return $this->sqlFrom ?? "order_items";
    }
     ...
     */
    public function clearCache(Entity\OrderItem $row, LifecycleEventArgs $args): void
    {
        // Clear lookup cache
    ...
    protected function loadDbValues(?BaseEntity $row)
    {
        ...
        $this->order_id->DbValue = $row->getOrderId();
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("OrderItemsList");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "OrderItemsView" => $this->language->phrase("View"),
            "OrderItemsEdit" => $this->language->phrase("Edit"),
            "OrderItemsAdd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getViewUrl(string $param = ""): string
    {
        ...
        $url = $this->keyUrl("OrderItemsView", $params);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("OrderItemsView", [$param]);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = BuildUrl("OrderItemsAdd", $param);
        return $this->addMasterUrl($url);
    }
    ...
    public function getEditUrl(string $param = ""): string
    {
        ...
        $url = $this->keyUrl("OrderItemsEdit", $params);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("OrderItemsList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("OrderItemsEdit", [$param]);
        return $this->addMasterUrl($url);
    }
    ...
    public function getCopyUrl(string $param = ""): string
    {
        ...
        $url = $this->keyUrl("OrderItemsAdd", $params);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("OrderItemsList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $param = ""): string
    {
        if ($this->UseAjaxActions && IsInfiniteScroll() && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("OrderItemsDelete", $param);
        }
    }
    ...
    public function renderListContent(array|BaseEntity $data, int $totalRecords)
    {
        global $httpContext;
        $container = Container();
        $page = $container->get("OrderItemsList");
        if (is_array($data)) {
    ...
    public function updateLastInsertId(Entity\OrderItem $row, LifecycleEventArgs $args)
    {
        $this->id->setDbValue($row['id']);
    }

    // Save entity change set
    public function preUpdate(Entity\OrderItem $row, PreUpdateEventArgs $args): void
    {
    ...
    public function preRemove(Entity\OrderItem $row, LifecycleEventArgs $args): void
    {
