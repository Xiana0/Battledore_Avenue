...
#[AsAlias("Cart", true)]
#[AsAlias("cart", true)]
#[AsEntityListener(event: Events::postPersist, method: 'clearCache', entity: Entity\Cart::class)]
#[AsEntityListener(event: Events::postUpdate, method: 'clearCache', entity: Entity\Cart::class)]
#[AsEntityListener(event: Events::postRemove, method: 'clearCache', entity: Entity\Cart::class)]
#[AsEntityListener(event: Events::postPersist, method: 'updateLastInsertId', entity: Entity\Cart::class)]
#[AsEntityListener(event: Events::preUpdate, method: 'preUpdate', entity: Entity\Cart::class)]
class Cart extends DbTable implements LookupTableInterface
{
    ...
    public DbField $product_name;
    public DbField $product_type;
    ...
    public function __construct(
        ...
        $this->EntityClass = Entity\Cart::class;
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // product_name
        $this->product_name = $this->Fields['product_name'];

        // product_type
        $this->product_type = $this->Fields['product_type'];

        // quantity
        $this->quantity = $this->Fields['quantity'];
    ...
    public function getFieldDefinitions(): array
    {
        return [
            ...
            'product_name' => [
                'FieldVar' => 'x_product_name', // Field variable name
                'Param' => 'product_name', // Field parameter name (Table class property name)
                'PropertyName' => 'productName', // Field entity property name
                'Expression' => '`product_name`', // Field expression (used in SQL)
                'BasicSearchExpression' => '`product_name`', // Field expression (used in basic search SQL)
                'Type' => 200, // Field type
                'DataType' => DataType::STRING, // Field data type (DataType::XXX)
                'ParameterType' => ParameterType::STRING, // Field Doctrine parameter type
                'Size' => 100, // Field size
                'DateTimeFormat' => -1, // Date time format
                'VirtualExpression' => '`product_name`', // Virtual field expression (used in ListSQL)
                'IsVirtual' => false, // Virtual field
                'ForceSelection' => false, // Autosuggest force selection
                'VirtualSearch' => false, // Search as virtual field
                'ViewTag' => 'FORMATTED TEXT', // View Tag
            ...
            'product_type' => [
                'FieldVar' => 'x_product_type', // Field variable name
                'Param' => 'product_type', // Field parameter name (Table class property name)
                'PropertyName' => 'productType', // Field entity property name
                'Expression' => '`product_type`', // Field expression (used in SQL)
                'BasicSearchExpression' => '`product_type`', // Field expression (used in basic search SQL)
                'Type' => 200, // Field type
                'DataType' => DataType::STRING, // Field data type (DataType::XXX)
                'ParameterType' => ParameterType::STRING, // Field Doctrine parameter type
                'Size' => 50, // Field size
                'DateTimeFormat' => -1, // Date time format
                'VirtualExpression' => '`product_type`', // Virtual field expression (used in ListSQL)
                'IsVirtual' => false, // Virtual field
                'ForceSelection' => false, // Autosuggest force selection
                'VirtualSearch' => false, // Search as virtual field
                'ViewTag' => 'FORMATTED TEXT', // View Tag
            ...
            'quantity' => [
                'FieldVar' => 'x_quantity', // Field variable name
                'Param' => 'quantity', // Field parameter name (Table class property name)
                'PropertyName' => 'quantity', // Field entity property name
                'Expression' => '`quantity`', // Field expression (used in SQL)
                'BasicSearchExpression' => '`quantity`', // Field expression (used in basic search SQL)
                'Type' => 3, // Field type
                'DataType' => DataType::NUMBER, // Field data type (DataType::XXX)
                'ParameterType' => ParameterType::INTEGER, // Field Doctrine parameter type
                'Size' => 11, // Field size
                'DateTimeFormat' => -1, // Date time format
                'VirtualExpression' => '`quantity`', // Virtual field expression (used in ListSQL)
                'IsVirtual' => false, // Virtual field
                'ForceSelection' => false, // Autosuggest force selection
                'VirtualSearch' => false, // Search as virtual field
                'ViewTag' => 'FORMATTED TEXT', // View Tag
     ...
     */
    public function clearCache(Entity\Cart $row, LifecycleEventArgs $args): void
    {
        // Clear lookup cache
    ...
    protected function loadDbValues(?BaseEntity $row)
    {
        ...
        $this->product_name->DbValue = $row->getProductName();
        $this->product_type->DbValue = $row->getProductType();
        $this->quantity->DbValue = $row->getQuantity();
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("CartList");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "CartView" => $this->language->phrase("View"),
            "CartEdit" => $this->language->phrase("Edit"),
            "CartAdd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getViewUrl(string $param = ""): string
    {
        ...
        $url = $this->keyUrl("CartView", $params);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("CartView", [$param]);
        return $this->addMasterUrl($url);
    }
    ...
    public function getEditUrl(string $param = ""): string
    {
        ...
        $url = $this->keyUrl("CartEdit", $params);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("CartList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("CartEdit", [$param]);
        return $this->addMasterUrl($url);
    }
    ...
    public function getCopyUrl(string $param = ""): string
    {
        ...
        $url = $this->keyUrl("CartAdd", $params);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("CartList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $param = ""): string
    {
        if ($this->UseAjaxActions && IsInfiniteScroll() && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("CartDelete", $param);
        }
    }
    ...
    public function updateLastInsertId(Entity\Cart $row, LifecycleEventArgs $args)
    {
        $this->id->setDbValue($row['id']);
    }

    // Save entity change set
    public function preUpdate(Entity\Cart $row, PreUpdateEventArgs $args): void
    {
    ...
    public function preRemove(Entity\Cart $row, LifecycleEventArgs $args): void
    {
