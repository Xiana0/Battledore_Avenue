 ...
 * Page class
 */
#[AsAlias("BookingsList", true)]
class BookingsList extends Bookings implements PageInterface
{
    use MessagesTrait;
    use FormTrait;
    ...
    public string $FormName = "fbookingslist";

    // CSS class/style
    public string $CurrentPageName = "BookingsList"; // Route action
    ...
    public function __construct(
        ...
        $this->MultiDeleteUrl = "BookingsDelete";
        $this->MultiUpdateUrl = "BookingsUpdate";
    ...
    public function getFilterList(): string
    {
        ...
        $filterList = Concat($filterList, $this->court_name->AdvancedSearch->toJson(), ","); // Field court_name
        $filterList = Concat($filterList, $this->booking_date->AdvancedSearch->toJson(), ","); // Field booking_date
        $filterList = Concat($filterList, $this->booking_time->AdvancedSearch->toJson(), ","); // Field booking_time
    ...
    protected function processFilterList(): bool
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fbookingssrch", $filters);
    ...
    protected function restoreFilterList(): void
    {
        ...
        $this->applyFilterToAdvancedSearch("user_id", $filter); // user_id
        $this->applyFilterToAdvancedSearch("court_name", $filter); // court_name
        $this->applyFilterToAdvancedSearch("booking_date", $filter); // booking_date
        $this->applyFilterToAdvancedSearch("booking_time", $filter); // booking_time
    ...
    public function basicSearchWhere(bool $default = false): string
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = $this->court_name;
        $searchFlds[] = $this->booking_time;
    ...
    protected function setupSortOrder(): void
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->court_name); // court_name
            $this->updateSort($this->booking_date); // booking_date
            $this->updateSort($this->booking_time); // booking_time
    ...
    protected function resetCmd(): void
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->court_name->setSort("");
                $this->booking_date->setSort("");
                $this->booking_time->setSort("");
                $this->status->setSort("");
            }
        ...
        if ($this->CurrentMode == "view") { // Check view mode
            ...
            if (true) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"bookings\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-key=\"" . $oldKey . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $this->language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if (true) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editCaption . "\" data-table=\"bookings\" data-caption=\"" . $editCaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-key=\"" . $oldKey . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $this->language->phrase("EditLink") . "</a>";
                } else {
            ...
            if (true) {
                if ($this->ModalAdd && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copyCaption . "\" data-table=\"bookings\" data-caption=\"" . $copyCaption . "\" data-ew-action=\"modal\" data-action=\"copy\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-key=\"" . $oldKey . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-btn=\"AddBtn\">" . $this->language->phrase("CopyLink") . "</a>";
                } else {
            ...
            if (true) {
                ...
                if ($this->UseAjaxActions && $this->PageID != "grid") {
                    $opt->Body = "<a class=\"ew-row-link ew-delete\" data-ew-action=\"inline\" data-action=\"delete\" data-table=\"bookings\" title=\"" . $deleteTitle . "\" data-caption=\"" . $deleteTitle . "\" data-key=\"" . $oldKey . "\" data-url=\"" . HtmlEncode(GetUrl($this->DeleteUrl)) . "\">" . $deleteCaption . "</a>";
                } else {
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fbookingslist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fbookingslist\" data-key=\"" . $this->keyToJson(true) .
    ...
    protected function setupOtherOptions(): void
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"bookings\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $this->language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "court_name");
            $this->createColumnOption($option, "booking_date");
            $this->createColumnOption($option, "booking_time");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fbookingssrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = $this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fbookingssrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions(): void
    {
        ...
        foreach ($this->ListActions as $listAction) { // ActionType::MULTIPLE
            if ($listAction->Select == ActionType::MULTIPLE && $listAction->getVisible()) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fbookingslist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
                $item->Visible = true;
            }
        }
    ...
    protected function setupRow(): void
    {
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_bookings", "data-rowtype" => RowType::ADD]);
    ...
    public function renderRow(RowType $rowType = RowType::VIEW, bool $resetAttributes = true): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->booking_time->TooltipValue = "";
    ...
    protected function setupSearchOptions(): void
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fbookingssrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $this->language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
