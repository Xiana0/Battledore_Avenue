 ...
 * Page class
 */
#[AsAlias("AccessoriesList", true)]
class AccessoriesList extends Accessories implements PageInterface
{
    use MessagesTrait;
    use FormTrait;
    ...
    public string $FormName = "faccessorieslist";

    // CSS class/style
    public string $CurrentPageName = "AccessoriesList"; // Route action
    ...
    public function __construct(
        ...
        $this->MultiDeleteUrl = "AccessoriesDelete";
        $this->MultiUpdateUrl = "AccessoriesUpdate";
    ...
    public function setVisibility(): void
    {
        ...
        $this->accessory_name->setVisibility();
        ...
        if ($url !== null) {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "AccessoriesView"); // If View page, no primary button
                } else { // List page
    ...
    public function run(): void
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "faccessoriesgrid";
        }
        ...
        if ($this->isGridAdd()) {
            ...
            $this->TotalRecords = $this->DisplayRecords;
            $this->StopRecord = $this->DisplayRecords;
        } elseif (!(IsApi() && ($this->isExport() || IsExport()))) {
        ...
        // Set X-Return-Url in header if necessary
    ...
    public function getFilterList(): string
    {
        ...
        $filterList = Concat($filterList, $this->accessory_name->AdvancedSearch->toJson(), ","); // Field accessory_name
    ...
    protected function processFilterList(): bool
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("faccessoriessrch", $filters);
    ...
    protected function restoreFilterList(): void
    {
        ...
        $this->Command = "search";
        $this->applyFilterToAdvancedSearch("id", $filter); // id
        $this->applyFilterToAdvancedSearch("accessory_name", $filter); // accessory_name
        $this->applyFilterToAdvancedSearch("price", $filter); // price
        $this->applyFilterToAdvancedSearch("image", $filter); // image
        $this->applyFilterToAdvancedSearch("stock", $filter); // stock
    ...
    public function basicSearchWhere(bool $default = false): string
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = $this->accessory_name;
    ...
    protected function setupSortOrder(): void
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->accessory_name); // accessory_name
    ...
    public function renderListOptions(): void
    {
        ...
        if ($this->CurrentMode == "view") { // Check view mode
            ...
            $oldKey = HtmlEncode($this->getOldKeyAsString());
            ...
            if (true) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"accessories\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-key=\"" . $oldKey . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $this->language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if (true) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editCaption . "\" data-table=\"accessories\" data-caption=\"" . $editCaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-key=\"" . $oldKey . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $this->language->phrase("EditLink") . "</a>";
                } else {
            ...
            if (true) {
                if ($this->ModalAdd && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copyCaption . "\" data-table=\"accessories\" data-caption=\"" . $copyCaption . "\" data-ew-action=\"modal\" data-action=\"copy\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-key=\"" . $oldKey . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-btn=\"AddBtn\">" . $this->language->phrase("CopyLink") . "</a>";
                } else {
            ...
            if (true) {
                ...
                if ($this->UseAjaxActions && $this->PageID != "grid") {
                    $opt->Body = "<a class=\"ew-row-link ew-delete\" data-ew-action=\"inline\" data-action=\"delete\" data-table=\"accessories\" title=\"" . $deleteTitle . "\" data-caption=\"" . $deleteTitle . "\" data-key=\"" . $oldKey . "\" data-url=\"" . HtmlEncode(GetUrl($this->DeleteUrl)) . "\">" . $deleteCaption . "</a>";
                } else {
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"faccessorieslist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"faccessorieslist\" data-key=\"" . $this->keyToJson(true) .
    ...
    protected function setupOtherOptions(): void
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"accessories\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $this->language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "accessory_name");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"faccessoriessrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = $this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"faccessoriessrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions(): void
    {
        ...
        foreach ($this->ListActions as $listAction) { // ActionType::MULTIPLE
            if ($listAction->Select == ActionType::MULTIPLE && $listAction->getVisible()) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="faccessorieslist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
                $item->Visible = true;
            }
        }
    ...
    protected function processListAction(): bool
    {
        ...
        $isAjax = Post("ajax") == $userAction;
        if (
            IsEmpty(Session(AddTabId(SESSION_INLINE_MODE))) // Make sure not grid/inline mode
            && $filter != ""
            && $userAction != ""
        ) {
            ...
            if ($listAction) {
                ...
                if (!$listAction->Allowed) {
                    ...
                    if ($isAjax) { // Ajax
                        $this->Response = new Response("<p class=\"text-danger\">" . $errmsg . "</p>");
                        return true;
                    } else {
            ...
            if ($this->SelectedCount > 0) {
                ...
                if ($processed) {
                    ...
                    // Load records again if non-Ajax
                    if (!$isAjax) {
                        $this->getEntityManager()->clear(); // Force a fresh read of entities
                        $this->Records = [];
                    }
                } else {
            ...
            if ($isAjax) { // Ajax
                if (IsJsonResponse($this->Response)) { // List action returns JSON
                    ...
                    if ($this->peekSuccessMessage()) {
                        $this->Response = new Response("<p class=\"text-success\">" . $this->getSuccessMessage() . "</p>");
                    }
                    if ($this->peekFailureMessage()) {
                        $this->Response = new Response("<p class=\"text-danger\">" . $this->getFailureMessage() . "</p>");
                    }
                }
                return true;
            }
    ...
    protected function setupRow(): void
    {
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_accessories", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_accessories",
    ...
    public function loadRowValues(?BaseEntity $row = null): void
    {
        ...
        $this->accessory_name->setDbValue($row['accessory_name']);
    ...
    protected function newRow(): BaseEntity
    {
        ...
        if (!IsEmpty($this->accessory_name->DefaultValue)) {
            $row['accessory_name'] = strval($this->accessory_name->DefaultValue);
        }
    ...
    public function renderRow(RowType $rowType = RowType::VIEW, bool $resetAttributes = true): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // accessory_name
            $this->accessory_name->ViewValue = $this->accessory_name->CurrentValue;
            ...
            // accessory_name
            $this->accessory_name->HrefValue = "";
            $this->accessory_name->TooltipValue = "";
    ...
    protected function setupSearchOptions(): void
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"faccessoriessrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $this->language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
