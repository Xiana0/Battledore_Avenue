 ...
 */
#[AsAlias("Bookings", true)]
#[AsAlias("bookings", true)]
#[AsEntityListener(event: Events::postPersist, method: 'clearCache', entity: Entity\Booking::class)]
#[AsEntityListener(event: Events::postUpdate, method: 'clearCache', entity: Entity\Booking::class)]
#[AsEntityListener(event: Events::postRemove, method: 'clearCache', entity: Entity\Booking::class)]
#[AsEntityListener(event: Events::postPersist, method: 'updateLastInsertId', entity: Entity\Booking::class)]
#[AsEntityListener(event: Events::preUpdate, method: 'preUpdate', entity: Entity\Booking::class)]
class Bookings extends DbTable implements LookupTableInterface
{
    ...
    public DbField $court_name;
    public DbField $booking_date;
    public DbField $booking_time;
    ...
    public function __construct(
        ...
        $this->EntityClass = Entity\Booking::class;
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // court_name
        $this->court_name = $this->Fields['court_name'];

        // booking_date
        $this->booking_date = $this->Fields['booking_date'];
        $this->booking_date->DefaultErrorMessage = str_replace("%s", $httpContext["DATE_FORMAT"], $this->language->phrase("IncorrectDate"));

        // booking_time
        $this->booking_time = $this->Fields['booking_time'];
    ...
    public function getFieldDefinitions(): array
    {
        return [
            ...
            'court_name' => [
                'FieldVar' => 'x_court_name', // Field variable name
                'Param' => 'court_name', // Field parameter name (Table class property name)
                'PropertyName' => 'courtName', // Field entity property name
                'Expression' => '`court_name`', // Field expression (used in SQL)
                'BasicSearchExpression' => '`court_name`', // Field expression (used in basic search SQL)
                'Type' => 200, // Field type
                'DataType' => DataType::STRING, // Field data type (DataType::XXX)
                'ParameterType' => ParameterType::STRING, // Field Doctrine parameter type
                'Size' => 50, // Field size
                'DateTimeFormat' => -1, // Date time format
                'VirtualExpression' => '`court_name`', // Virtual field expression (used in ListSQL)
                'IsVirtual' => false, // Virtual field
                'ForceSelection' => false, // Autosuggest force selection
                'VirtualSearch' => false, // Search as virtual field
                'ViewTag' => 'FORMATTED TEXT', // View Tag
            ...
            'booking_date' => [
                'FieldVar' => 'x_booking_date', // Field variable name
                'Param' => 'booking_date', // Field parameter name (Table class property name)
                'PropertyName' => 'bookingDate', // Field entity property name
                'Expression' => '`booking_date`', // Field expression (used in SQL)
                'BasicSearchExpression' => CastDateFieldForLike("`booking_date`", 0, "DB"), // Field expression (used in basic search SQL)
                'Type' => 133, // Field type
                'DataType' => DataType::DATE, // Field data type (DataType::XXX)
                'ParameterType' => ParameterType::STRING, // Field Doctrine parameter type
                'Size' => 10, // Field size
                'DateTimeFormat' => 0, // Date time format
                'VirtualExpression' => '`booking_date`', // Virtual field expression (used in ListSQL)
                'IsVirtual' => false, // Virtual field
                'ForceSelection' => false, // Autosuggest force selection
                'VirtualSearch' => false, // Search as virtual field
                'ViewTag' => 'FORMATTED TEXT', // View Tag
            ...
            'booking_time' => [
                'FieldVar' => 'x_booking_time', // Field variable name
                'Param' => 'booking_time', // Field parameter name (Table class property name)
                'PropertyName' => 'bookingTime', // Field entity property name
                'Expression' => '`booking_time`', // Field expression (used in SQL)
                'BasicSearchExpression' => '`booking_time`', // Field expression (used in basic search SQL)
                'Type' => 200, // Field type
                'DataType' => DataType::STRING, // Field data type (DataType::XXX)
                'ParameterType' => ParameterType::STRING, // Field Doctrine parameter type
                'Size' => 50, // Field size
                'DateTimeFormat' => -1, // Date time format
                'VirtualExpression' => '`booking_time`', // Virtual field expression (used in ListSQL)
                'IsVirtual' => false, // Virtual field
                'ForceSelection' => false, // Autosuggest force selection
                'VirtualSearch' => false, // Search as virtual field
                'ViewTag' => 'FORMATTED TEXT', // View Tag
     ...
     */
    public function clearCache(Entity\Booking $row, LifecycleEventArgs $args): void
    {
        // Clear lookup cache
    ...
    protected function loadDbValues(?BaseEntity $row)
    {
        ...
        $this->court_name->DbValue = $row->getCourtName();
        $this->booking_date->DbValue = $row->getBookingDate();
        $this->booking_time->DbValue = $row->getBookingTime();
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("BookingsList");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "BookingsView" => $this->language->phrase("View"),
            "BookingsEdit" => $this->language->phrase("Edit"),
            "BookingsAdd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getViewUrl(string $param = ""): string
    {
        ...
        $url = $this->keyUrl("BookingsView", $params);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("BookingsView", [$param]);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = BuildUrl("BookingsAdd", $param);
        return $this->addMasterUrl($url);
    }
    ...
    public function getEditUrl(string $param = ""): string
    {
        ...
        $url = $this->keyUrl("BookingsEdit", $params);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("BookingsList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("BookingsEdit", [$param]);
        return $this->addMasterUrl($url);
    }
    ...
    public function getCopyUrl(string $param = ""): string
    {
        ...
        $url = $this->keyUrl("BookingsAdd", $params);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("BookingsList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $param = ""): string
    {
        if ($this->UseAjaxActions && IsInfiniteScroll() && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("BookingsDelete", $param);
        }
    }
    ...
    public function renderListContent(array|BaseEntity $data, int $totalRecords)
    {
        global $httpContext;
        $container = Container();
        $page = $container->get("BookingsList");
        if (is_array($data)) {
        ...
        if (!$doc->ExportCustom && $writeHeader) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->court_name);
                    $doc->exportCaption($this->booking_date);
                    $doc->exportCaption($this->booking_time);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->court_name);
                        $doc->exportField($this->booking_date);
                        $doc->exportField($this->booking_time);
    ...
    public function updateLastInsertId(Entity\Booking $row, LifecycleEventArgs $args)
    {
        $this->id->setDbValue($row['id']);
    }

    // Save entity change set
    public function preUpdate(Entity\Booking $row, PreUpdateEventArgs $args): void
    {
    ...
    public function preRemove(Entity\Booking $row, LifecycleEventArgs $args): void
    {
